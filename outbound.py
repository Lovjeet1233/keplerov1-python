...
if __name__ == "__main__":
    # Note: Removing agent_name allows the worker to auto-dispatch to any room
    # The supervisor room will be created separately and won't conflict
    cli.run_app(
        WorkerOptions(
            entrypoint_fnc=entrypoint,
            # agent_name="sip-inbound",  # Commented out to allow auto-dispatch
            initialize_process_timeout=30.0,  # Increase timeout
            # Disable inference executor for Windows compatibility
            # The IPC mechanism uses Unix-specific features that don't work on Windows
        ),
    )
